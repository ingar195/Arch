...
if __name__ == "__main__":
    ...
    if user == "fw":
        ...
        create_dir(os.path.join(home_dir, ".config/i3"))
        create_dir(os.path.join(home_dir, ".config/polybar"))

        if in_file(os.path.join(home_dir, ".config/i3/config"), "# This file has been auto-generated by i3-config-wizard(1)."):
            logging.info("deleting default config files")
    ...
    if not file_exists(os.path.join(home_dir, ".dotfiles/config")):
        ...
        run_program(f"{dotfiles_command} reset --hard")
    ...
    logging.warning("Uncomment before release")
